...
from Models.SCC.helper import split_by_id_ratio_1_to_4  # make sure this is correctly defined
...
def extract_sequences(df):
    ...
    for _, row in df.iterrows():
        ...
        if len(steps) >= 2:
            ...
            label = float(row["label"])  # ✅ use existing label from the row
...
print(f"Train class distribution: {Counter(train_targets)}")
print(f"Val class distribution: {Counter(val_targets)}")
...
train_loader = DataLoader(LGSCCDataset(scaled_train, train_targets), batch_size=128, shuffle=True, collate_fn=collate_fn)
val_loader = DataLoader(LGSCCDataset(scaled_val, val_targets), batch_size=128, collate_fn=collate_fn)
...
class TransformerModel(nn.Module):
    def __init__(self, input_size=5, d_model=32, nhead=4, layers=2, dropout=0.1):
        super().__init__()
...
patience, best_loss, no_improve = 5, float("inf"), 0
