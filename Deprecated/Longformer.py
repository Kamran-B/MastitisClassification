...
'''Based on testing Longformer did not improve accuracy, made it worse actually.
Also took significantly longer to run. Improved on by custom transformer.'''
...
def main(seed_value=42, epochs=4, printStats=True, savePerf=False):
    ...
    class GeneticDataset(Dataset):
        def __init__(self, snp_sequences, labels, tokenizer, max_length=4096):
        ...
        def __getitem__(self, idx):
            ...
            # Convert SNP sequence to a string (no chunking needed now)
            snp_sequence_str = " ".join(map(str, snp_sequence))
    ...
    class CustomLongformerModel(nn.Module):
        def __init__(self, embedding_dim, hidden_dim, num_labels=2):
            ...
            self.max_length = 4096  # Longformer allows sequences up to 4096 tokens
            ...
            self.fc = nn.Linear(self.longformer.config.hidden_size + 2 * embedding_dim, hidden_dim)
        ...
        def forward(self, snp_sequences, breed_ids, herd_year_ids):
            device = next(self.parameters()).device

            # Tokenize the entire sequence at once, no chunking required
            encodings = self.tokenizer(snp_sequences, padding="max_length", truncation=True, max_length=self.max_length,
                                       return_tensors="pt")
            encodings = {k: v.to(device) for k, v in encodings.items()}

            # Get the Longformer outputs
            outputs = self.longformer(**encodings)
            snp_pooled_output = outputs.pooler_output
            ...
            combined_features = torch.cat((snp_pooled_output, breed_embeds, herd_year_embeds), dim=-1)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            # SNP sequence (no chunking now)
            snp_sequences = batch['snp_sequence']
            breed_ids = batch['breed'].to(device)
            herd_year_ids = batch['herd_year'].to(device)
            labels = batch['labels'].to(device)

            # Forward pass
            outputs = model(snp_sequences, breed_ids, herd_year_ids)
