...
def explain_model_with_shap(model, test_loader, tokenizer, device, num_samples=100):
    model.eval()  # Ensure model is in evaluation mode

    # Select a small subset for SHAP explanation
    X_explain, _ = next(iter(test_loader))  # Get a batch
    X_explain = {k: v.to(device) for k, v in X_explain.items()}

    # Define SHAP Explainer
    explainer = shap.GradientExplainer(model, X_explain)

    # Compute SHAP values
    shap_values = explainer.shap_values(X_explain, nsamples=num_samples)

    # Convert SNP indices back to original SNP IDs (if necessary)
    feature_names = [f"SNP_{i}" for i in range(len(X_explain['snp_chunks'][0]))]

    # Generate summary plot
    shap.summary_plot(shap_values, features=X_explain['snp_chunks'].cpu().numpy(), feature_names=feature_names)
...
def main(seed, epochs, printStats=True, savePerf=False, top_snps=None):
    ...
    accuracies = train_and_eval(model, device, train_loader, test_loader, optimizer, loss_fn, epochs, printStats, savePerf,
                          top_performances)
    explain_model_with_shap(model, test_loader, tokenizer, device, num_samples=100)
...
if __name__ == "__main__":
    EvalScript(1, "Data/TopSNPs/MutualInfo/top100_SNPs_mi_binary.txt", "Logging/Transformer/mi_top100.json")
